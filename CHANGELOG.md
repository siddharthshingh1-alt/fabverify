...
## [2026-08-30 · Part 1a] — Fix a regression Part 1 introduced: the stuck timer fired on the SUCCESS path

> Self-inflicted, found by re-reading the committed code after the founder reported “dashboard loads, then bounces to /login a few seconds later” — a symptom Part 1 itself produced.

### Fixed
- ** /  armed a 2.5s stuck timer and disarmed it only in the effect CLEANUP.** A React cleanup runs when the effect re-runs or the component unmounts — neither happens while a user simply sits on a resolved page. So the guard resolved, rendered the dashboard, and the timer fired anyway, wiping the identity mirrors and hard-navigating to . **This affected EVERY guarded page load, not just the first-login failure Part 1 was written for** — worse than the bug it was softening.
- ⚠️ **THE RULE: a decision must disarm the timer SYNCHRONOUSLY, at the moment it is made. Never rely on a cleanup to cancel something that must not fire.**
- Timer discipline extracted into **** in  — shared by both hooks, with two independent stops ( **and** a  flag the callback re-checks), and  called synchronously on every decision branch. Armed only on the genuinely-undecided branch ().

### The test gap — the more important half
- ⚠️ **THE 18/18 SUITE WAS GREEN WHILE THIS DEFECT SHIPPED.** Every assertion tested WHAT  does; **none tested WHEN it is called.** A green suite that does not cover the risky part buys false confidence, which is worse than no suite at all.
- **New section [G], 7 assertions on TIMING** — settled timer does not navigate; un-settled one does; cancelled one does not;  idempotent; settling after firing does not re-fire; and **“a resolved guard survives a long idle page view”, which is the shipped symptom stated directly as an assertion.**
- ✅ **PROVEN TO CATCH IT: the defect was deliberately reintroduced and the suite went RED on exactly those assertions (21/25, exit 1), then green again (25/25) once restored.** A regression test that has never failed is a guess.

### Also fixed
-  broke 
> fabverify@0.1.0 build
> next build
...
✓ Compiled successfully in 34.2s
  ...
  Generating static pages using 7 workers (0/162) ...
  Generating static pages using 7 workers (40/162) 
  Generating static pages using 7 workers (80/162) 
  Generating static pages using 7 workers (121/162) 
✓ Generating static pages using 7 workers (162/162) in 3.2s
...
├ ○ /_not-found
├ ○ /analytics
├ ƒ /api/account/password
├ ƒ /api/account/password-status
├ ƒ /api/auth/otp/send
├ ƒ /api/auth/password-login
├ ƒ /api/auth/password-reset
├ ƒ /api/conversations
├ ƒ /api/dev-auth/lookup
├ ƒ /api/dev-auth/save-profile
├ ƒ /api/dev-auth/save-user-type
├ ƒ /api/enquiries
├ ƒ /api/manufacturer-profile
├ ƒ /api/manufacturers
├ ƒ /api/manufacturers/[id]
├ ƒ /api/messages
├ ƒ /api/messages/read
├ ƒ /api/orders
├ ƒ /api/orders/[id]
...
├ ○ /jobworker/analytics
├ ○ /jobworker/clients
├ ƒ /jobworker/clients/[id]
├ ○ /jobworker/credit
├ ○ /jobworker/dashboard
├ ○ /jobworker/enquiries
├ ○ /jobworker/fabprice
├ ○ /jobworker/orders
├ ƒ /jobworker/orders/[id]
├ ○ /jobworker/profile
├ ○ /jobworker/verification
├ ○ /login
├ ○ /manufacturer/analytics
├ ○ /manufacturer/buyers
├ ƒ /manufacturer/buyers/[id]
├ ○ /manufacturer/credit
├ ○ /manufacturer/dashboard
├ ○ /manufacturer/enquiries
├ ○ /manufacturer/fabprice
├ ○ /manufacturer/orders
├ ƒ /manufacturer/orders/[id]
...
├ ○ /supplier/analytics
├ ○ /supplier/buyers
├ ƒ /supplier/buyers/[id]
├ ○ /supplier/credit
├ ○ /supplier/dashboard
├ ○ /supplier/enquiries
├ ○ /supplier/fabprice
├ ○ /supplier/orders
├ ƒ /supplier/orders/[id]
├ ○ /supplier/profile
├ ○ /supplier/samples
├ ○ /supplier/verification
├ ○ /talent/designer/analytics
├ ○ /talent/designer/clients
├ ƒ /talent/designer/clients/[id]
├ ○ /talent/designer/credit
├ ○ /talent/designer/dashboard
├ ○ /talent/designer/enquiries
├ ○ /talent/designer/fabprice
├ ○ /talent/designer/profile
├ ○ /talent/designer/projects
├ ƒ /talent/designer/projects/[id]
├ ○ /talent/designer/verification
├ ○ /talent/master/clients
├ ƒ /talent/master/clients/[id]
├ ○ /talent/master/credit
├ ○ /talent/master/dashboard
├ ○ /talent/master/enquiries
├ ○ /talent/master/fabprice
├ ○ /talent/master/profile
├ ○ /talent/master/projects
├ ƒ /talent/master/projects/[id]
├ ○ /talent/master/verification
├ ○ /talent/merchandiser/clients
├ ƒ /talent/merchandiser/clients/[id]
├ ○ /talent/merchandiser/credit
├ ○ /talent/merchandiser/dashboard
├ ○ /talent/merchandiser/enquiries
├ ○ /talent/merchandiser/fabprice
├ ○ /talent/merchandiser/profile
├ ○ /talent/merchandiser/projects
├ ƒ /talent/merchandiser/projects/[id]
├ ○ /talent/merchandiser/verification
├ ○ /talent/qc/clients
├ ƒ /talent/qc/clients/[id]
├ ○ /talent/qc/credit
├ ○ /talent/qc/dashboard
├ ○ /talent/qc/enquiries
├ ○ /talent/qc/fabprice
...
ƒ  (Dynamic)  server-rendered on demand (top-level  with no static import/export — needed ). Committed in . ⚠️ **Cause: the build was run BEFORE the last file landed and never re-run. Build LAST, after every file is in place.**

### Verified
- **25/25** (18 original + 7 timing) ·  silent · clean-rebuild 
> fabverify@0.1.0 build
> next build
...
✓ Compiled successfully in 31.4s
  ...
  Generating static pages using 7 workers (80/162) 
  Generating static pages using 7 workers (121/162) 
✓ Generating static pages using 7 workers (162/162) in 4.8s
  Finalizing page optimization ...
...
○  (Static)   prerendered as static content
ƒ  (Dynamic)  server-rendered on demand exit 0, 162 pages · eslint 0 errors.
- ⚠️ **ASSET-LEVEL check, not just the document:** every JS chunk on  (10),  (12) and  (11) returns **200**, and the Part 1a strings are confirmed in the bundles the browser actually downloads. **A prerendered HTML 200 proves nothing about whether the app works** — a stale  serves the document fine while 404ing every chunk, which cost a debugging round on 2026-08-30.  does not kill these processes on Windows; kill by port via .
