...
### Fixed
- **`useTypeGuard` and `useEnterpriseAccess` armed a 2.5s stuck timer and disarmed it only in the effect CLEANUP.** A React cleanup runs when the effect re-runs or the component unmounts — neither happens while a user simply sits on a resolved page. So the guard resolved, rendered the dashboard, and the timer fired anyway, wiping the identity mirrors and hard-navigating to `/login`. **This affected EVERY guarded page load, not just the first-login failure Part 1 was written for** — worse than the bug it was softening.
...
- Timer discipline extracted into **`armGuardStuckTimer(reason, delayMs)`** in `guardRecovery.ts` — shared by both hooks, with two independent stops (`clearTimeout` **and** a `settled` flag the callback re-checks), and `settle()` called synchronously on every decision branch. Armed only on the genuinely-undecided branch (`!mounted`).

### The test gap — the more important half
- ⚠️ **THE 18/18 SUITE WAS GREEN WHILE THIS DEFECT SHIPPED.** Every assertion tested WHAT `recoverToLogin` does; **none tested WHEN it is called.** A green suite that does not cover the risky part buys false confidence, which is worse than no suite at all.
- **New section [G], 7 assertions on TIMING** — settled timer does not navigate; un-settled one does; cancelled one does not; `settle()` idempotent; settling after firing does not re-fire; and **"a resolved guard survives a long idle page view", which is the shipped symptom stated directly as an assertion.**
...
### Also fixed
- `verify-guard-recovery.ts` broke `npm run build` (top-level `await` with no static import/export — needed `export {}`). Committed in `720d452`. ⚠️ **Cause: the build was run BEFORE the last file landed and never re-run. Build LAST, after every file is in place.**

### Verified
- **25/25** (18 original + 7 timing) · `tsc` silent · clean-rebuild `npm run build` exit 0, 162 pages · eslint 0 errors.
- ⚠️ **ASSET-LEVEL check, not just the document:** every JS chunk on `/login` (10), `/manufacturer/dashboard` (12) and `/onboarding/password` (11) returns **200**, and the Part 1a strings are confirmed present in the bundles the browser actually downloads.

### Two process lessons, recorded because both cost a round
- **A prerendered HTML 200 proves nothing about whether the app works.** A stale `next start` serves the document fine while 404ing every chunk. Check the assets, not the page.
- **`pkill` does not kill these processes on Windows** — kill by port via `Get-NetTCPConnection`. A silent `EADDRINUSE` leaves the OLD server answering, which looks exactly like a broken build.

### Still open
- 🛑 **The original blank-screen bug remains UNDIAGNOSED.** Run A's evidence was contaminated by this regression and must be re-run on clean ground. No Part 2 root-cause work until then. Merge stays blocked.
