...
| Password login option | ✅ | **LIVE AND PRODUCTION-PROVEN 2026-08-21.** Phone + password is the PRIMARY login; OTP is the fallback. Proven on the real LAN production build: password login reached the dashboard with real data loading (landing alone was not accepted — a token that mints but fails the ladder looks identical for half a second). The **mandatory set-password gate** ([I27]) forces every password-less account to set one before reaching the app, re-checked on EVERY app entry, verified on localhost for forced-redirect / abandon-and-return / no-loop / error-then-retry. Backend all proven separately: argon2id hashing (2.2), set-password (2.4), credential verify (2.5a), lockout (2.7), our own token issue+verify+ladder (2.5b). ✅ **2.6c IS DONE AND PRODUCTION-PROVEN (2026-08-24)** — the OTP send is now server-side, throttled and enumeration-uniform, and the D4 timing floor is MEASURED (4722 ms ceiling) and BINDING (6000 ms). Proven on the real LAN production build: SMS arrives · `verifyOtp` still succeeds after the send stopped creating a pre-verification session in the browser · set-password gate · password login · OTP fallback · throttle refuses the repeat. Suites 56/56 · 42/42 · 38/38. ⚠️ **STILL OPEN, IN ORDER: 2.6d** (OTP send-path latency — the floor currently pads around **2981 ms of our own sequential DB round trips**, 63% of the total; a 6 s reset is fine for a founder and not for real users, so this is a **HARD PREREQUISITE of 2.8b**) → **2.8b** (the reset ROUTE + UI + production test; **the reset LOGIC is already built — see the row below**) → **2.9** (docs sweep). ✅ **Password SPRAYING is HANDLED since chunk 2.10 (2026-08-27), production-proven** — `checkLoginSprayThrottle` blocks an address that has failed against **10 DISTINCT ACCOUNTS in 15 minutes**. ⚠️ **NOT per-IP rate limiting**, which [I23] refused because a naive attempt cap behind an office NAT locks out real users; this counts distinct failed accounts, a shape only a sprayer produces, and a **successful login clears that account's rows for that address** so an office never trips it ([I35]). It **fails OPEN and loudly** ([I36]) because fail-closed would take the whole platform's login down on a database blip. The block returns a **generic 401, not a 429**, so it never tells a sprayer they were detected. Proven live: a sprayed address blocked while a real user on another address logged in normally **and 2.6a merged without the decision that was supposed to gate it.** `/api/dev-auth/lookup` is still unauthenticated. ⚠️ **NEW-USER SIGNUP is covered STRUCTURALLY by this gate** — a fresh account is routed to `/onboarding/password` once onboarding creates the `users` row (it cannot happen sooner: `user_credentials.user_id` is a NOT NULL FK) — **but that path has never been run end-to-end on a genuinely new account. It is an inference from the design, not an observation.** |
| Password RESET (forgot password) | ✅ | **LIVE AND PRODUCTION-PROVEN 2026-08-27 (chunk 2.8b).** `/login` → Forgot password? → `/reset-password` → real SMS → code + new password → dashboard, run end to end on a real number against the LAN production build. `token_epoch` **0 → 1** (the eviction), no phantom account, no stray credential. ⚠️ **THE SUBMIT ENDPOINT IS THROTTLED ([I33])** — it is unauthenticated and gated only by a 6-digit code, and nothing counted verify attempts before 2.8b; sends and guesses are now counted separately by `purpose` so a failed guess cannot burn the victim's send budget. ⚠️ **AND `verifyOtp` NO LONGER RUNS ON THE SHARED ADMIN CLIENT ([I34])** — it saves a session on whatever client it touches, which would have downgraded every `db.ts` call in the process from service role to that user. Suites: `verify-password-reset-route` 34/34, `verify-otp-verify-throttle` 15/15. *(Rows below describe the 2.8a seam that this is built on.)* **LOGIC DONE AND PROVEN 2026-08-21 (chunk 2.8a, commit `267271c`, 40/40).** `resetPasswordByOtp` (`authProvider.server.ts:719`) + `resetUserCredential` (`db.ts:307`), with **zero route importers** — asserted, not assumed. ⚠️ **2.8 WAS SPLIT: 2.8a = the seam (done) · 2.8b = route + UI + production test (NOT BUILT).** ⚠️ **DO NOT REBUILD 2.8a** — for several hours on 2026-08-21 every status doc said reset was unstarted because that commit touched zero markdown; the split existed only in a commit message. *(Corrected in the docs-only pass later that day.)* **What it does:** verifies the OTP **SERVER-SIDE** (today's OTP login verifies in the browser, and a reset has no session to check — so "the browser says the code passed" would let anyone POST that claim for any number), then resolves, then validates policy, then writes the hash + **bumps `token_epoch`** + **clears the lockout**, all in ONE statement. ⚠️ **The epoch bump is the point, not bookkeeping** — without it a reset leaves every existing session alive and the owner recovers nothing. ⚠️ **It evicts OUR tokens ONLY: a stolen SUPABASE session SURVIVES a reset**, and every OTP login still mints one — **never describe reset as "ends all existing sessions."** ✅ **THE RESET-OTP SEND IS NOW SERVER-SIDE AND THROTTLED (2.6c, 2026-08-24)** — it was browser-direct and unthrottled, which is why 2.6c gated 2.8b: the reset UI would otherwise have published a free, unauthenticated SMS trigger. Suite section G behaved exactly as designed — **G1/G2 inverted the day 2.6c landed, and G4 (the fails-when-fixed timing tripwire) fired the day the D4 residual was measured** and is now inverted too. ⚠️ **The remaining gate on 2.8b is 2.6d (latency), not 2.6c** — the D4 floor binds at 6000 ms, and a 6 s reset must not reach real users. ⚠️ **No email reset links, ever** ([I10] / the `user_credentials` row above: no `reset_token` column) — we do not verify emails. ⚠️ **The account's security floor is the PHONE NUMBER** — a SIM swap takes the account regardless of password strength. Already true because OTP login exists; the password did not raise it. |
