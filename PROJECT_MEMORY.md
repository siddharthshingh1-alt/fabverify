...
| `otp_requests` | 🔴 | **DESIGNED AND WRITTEN 2026-08-22 (chunk 2.6c), NOT YET APPLIED — the table does NOT exist in the real database.** `supabase/migrations/004_otp_requests.sql` is written and `schema.sql` carries the block, but the DDL is a **manual Supabase SQL Editor paste** (no direct Postgres connection in `.env.local`; PostgREST cannot run DDL) and has not been run. The 2.6c code that reads it is uncommitted working-tree state. The OTP send throttle: `phone_hash` (**HMAC-SHA256 under a key derived from `SESSION_TOKEN_SECRET` — never the raw number**, because this table takes a row for every number that ASKS for a code including numbers with no account, which raw would be the largest non-user PII pile on the platform), nullable `ip_hash`, `purpose`, `created_at`. ⚠️ **No FK to `users` by design** — counting requests for numbers with no account is the entire point. Rows purge after 48h; it is a counter, **not an audit log**. RLS on with zero policies = deny-all. |
...
| Password login option | ✅ | **LIVE AND PRODUCTION-PROVEN 2026-08-21.** Phone + password is the PRIMARY login; OTP is the fallback. Proven on the real LAN production build: password login reached the dashboard with real data loading (landing alone was not accepted — a token that mints but fails the ladder looks identical for half a second). The **mandatory set-password gate** ([I27]) forces every password-less account to set one before reaching the app, re-checked on EVERY app entry, verified on localhost for forced-redirect / abandon-and-return / no-loop / error-then-retry. Backend all proven separately: argon2id hashing (2.2), set-password (2.4), credential verify (2.5a), lockout (2.7), our own token issue+verify+ladder (2.5b). ⚠️ **STILL OPEN, IN ORDER: 2.6c** (OTP request hardening — the send is still browser-direct, unthrottled, and SMSes unknown numbers; ⏸️ **its code was WRITTEN 2026-08-22 but is UNCOMMITTED, UNPROVEN and blocked on the `otp_requests` table being created by hand — `git log` shows nothing for it, `git status` shows 12 files. DO NOT REBUILD IT; see the ⏸️ PAUSED HERE block in TASKS.md**) → **2.8b** (the reset ROUTE + UI + production test; **the reset LOGIC is already built — see the row below**) → **2.9** (docs sweep). ⚠️ **Password SPRAYING is unhandled** (2.7 is per-account only; per-IP deliberately unbuilt, [I23]) **and 2.6a merged without the decision that was supposed to gate it.** `/api/dev-auth/lookup` is still unauthenticated. ⚠️ **NEW-USER SIGNUP is covered STRUCTURALLY by this gate** — a fresh account is routed to `/onboarding/password` once onboarding creates the `users` row (it cannot happen sooner: `user_credentials.user_id` is a NOT NULL FK) — **but that path has never been run end-to-end on a genuinely new account. It is an inference from the design, not an observation.** |
| Password RESET (forgot password) | 🟡 | **LOGIC DONE AND PROVEN 2026-08-21 (chunk 2.8a, commit `267271c`, 40/40). NO ROUTE, NO UI — no user can reach it.** `resetPasswordByOtp` (`authProvider.server.ts:719`) + `resetUserCredential` (`db.ts:307`), with **zero route importers** — asserted, not assumed. ⚠️ **2.8 WAS SPLIT: 2.8a = the seam (done) · 2.8b = route + UI + production test (NOT BUILT).** ⚠️ **DO NOT REBUILD 2.8a** — for several hours on 2026-08-21 every status doc said reset was unstarted because that commit touched zero markdown; the split existed only in a commit message. *(Corrected in the docs-only pass later that day.)* **What it does:** verifies the OTP **SERVER-SIDE** (today's OTP login verifies in the browser, and a reset has no session to check — so "the browser says the code passed" would let anyone POST that claim for any number), then resolves, then validates policy, then writes the hash + **bumps `token_epoch`** + **clears the lockout**, all in ONE statement. ⚠️ **The epoch bump is the point, not bookkeeping** — without it a reset leaves every existing session alive and the owner recovers nothing. ⚠️ **It evicts OUR tokens ONLY: a stolen SUPABASE session SURVIVES a reset**, and every OTP login still mints one — **never describe reset as "ends all existing sessions."** ⚠️ **The reset-OTP SEND is still browser-direct and unthrottled**, which is why **2.6c is a hard prerequisite of 2.8b** — the reset UI would otherwise publish a free, unauthenticated SMS trigger. Suite section G (G1) **fails the day 2.6c lands, by design.** ⚠️ **No email reset links, ever** ([I10] / the `user_credentials` row above: no `reset_token` column) — we do not verify emails. ⚠️ **The account's security floor is the PHONE NUMBER** — a SIM swap takes the account regardless of password strength. Already true because OTP login exists; the password did not raise it. |
