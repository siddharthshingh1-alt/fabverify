...
| Password login option | 🟡 | decided (M10). **2.0+2.1 done 2026-08-06; 2.2+2.3+2.4+2.5a done 2026-08-08.** ⚠️ **A user can SET a password, and the server can CHECK one — but nothing can LOG IN with one, and no screen reaches any of it.** The verifier `verifyPasswordCredential` has **zero route importers on purpose** ([I18]): an endpoint answering "are these credentials valid?" without issuing a session is a credential oracle, and its absence is what makes deferring lockout safe. Remaining: **2.5b** (issue/verify our own token), 2.6 (login UI), 2.7 (lockout). ⚠️ **2.7 must land WITH or BEFORE 2.6** — 2.6 is what opens the brute-force surface, and TASKS.md sequences 2.7 after it. ⚠️ M10 requires us to issue our OWN session tokens (Supabase will not sign a JWT for a credential it does not hold), which pulls A12 Phase 2 dual-verify forward; roughly half of M10 is the token subsystem, not the credential. Highest-risk chunks: ✅ **2.2 (argon2id hashing — done, 42/42)** and 🔴 **2.5b (our own token verify — a bug is an auth bypass; the last one)**. |
