...
| `user_credentials` | ✅ | **Created 2026-08-06 (chunk 2.1); FIRST WRITTEN 2026-08-08 (chunk 2.4).** ⚠️ **HAS REAL ROWS SINCE 2026-08-21** — the founder's enterprise account holds a real password, set through the real screen and used for a real production login (chunk 2.6a/2.6b). *(This entry previously read "currently 0 rows … nothing in the UI reaches the endpoint yet." True until 2026-08-21, then stale.)* Read/written by `db.ts getUserCredential` / `upsertUserCredential` (from `authProvider.server.ts setPassword`, from `POST /api/account/password`), by `recordFailedPasswordAttempt` / `clearFailedPasswordAttempts` since chunk 2.7, and by **`resetUserCredential` since chunk 2.8a** (from `resetPasswordByOtp` — hash + `token_epoch` bump + lockout clear in ONE statement). ⚠️ **`verifyPasswordCredential` NO LONGER HAS ZERO ROUTE IMPORTERS** — chunk 2.6a legitimately ended [I18]'s deferral by adding `POST /api/auth/password-login`. The suites now assert an **ALLOWLIST OF ONE** rather than zero, and C5 **walks the filesystem** because the original `git grep` check searched tracked files only and stayed green while the new route was untracked. Password hashes FabVerify owns (DECISIONS **M10**, **I10**). ⚠️ **A SEPARATE TABLE, NOT a `users.password_hash` column, and that is a security decision, not a style one:** `/api/dev-auth/lookup` is unauthenticated and returns `getUserByPhone(phone)` = `.select("*")` on `users` (`db.ts:38-39`), so a hash on `users` would be handed to an anonymous caller for any phone on the platform. A separate table cannot be reached by `select("*")` on `users` — the leak is impossible by construction. 12 columns: `id`, `user_id → users(id)` **ON DELETE CASCADE**, `credential_type` (no CHECK), `password_hash` (the argon2id ENCODED string — carries its own salt AND parameters, so no separate salt/params column), `token_epoch` (revocation, **I12**), `failed_attempts`/`last_failed_at`/`locked_until` (**LIVE since chunk 2.7, 2026-08-20** — 10 failures → a fixed 15-minute auto-expiring cooldown), `password_changed_at`/`must_change_password` (chunk 2.8a + enterprise default password), `created_at`, `updated_at` (application-maintained, not a trigger). `UNIQUE (user_id, credential_type)` — which is what makes "re-setting a password replaces rather than duplicates" structural, and, because it LEADS with `user_id`, why there is deliberately **no separate FK index** here unlike `auth_identities`. ⚠️ Every column beyond `user_id`/`password_hash` is nullable or NOT NULL-with-default, so all of it is inert until its chunk is built; the counters are NOT NULL DEFAULT 0 on purpose, since `failed_attempts + 1` is NULL on a nullable column — a lockout that never locks. ⚠️ **No `reset_token` column by design** — reset proves the phone by OTP; no email reset links. RLS on with **zero policies** = deny-all, **proven by an anon `INSERT` returning `42501`** run side by side with `auth_identities` as a control (an anon `SELECT` returns `200 []` on both and proves nothing on an empty table). In both `supabase/migrations/003_user_credentials.sql` and `supabase/schema.sql`. |

| `otp_requests` | ✅ | **CREATED 2026-08-23/24 AND LIVE — the table EXISTS and 2.6c is production-proven.** *(This row previously read "NOT YET APPLIED — the table does NOT exist". True until the founder ran the DDL by hand; stale after, and it briefly alarmed a session on 2026-08-24.)* Applied as a **manual Supabase SQL Editor paste** (no direct Postgres connection in `.env.local`; PostgREST cannot run DDL), from `supabase/migrations/004_otp_requests.sql`; `schema.sql` carries the block. **RLS deny-all PROVEN FROM OUTSIDE**, not assumed: an anon `INSERT` returns **`42501`** while an anon `SELECT` returns `200` — the SQL Editor bypasses RLS and can never prove this. The OTP send throttle: `phone_hash` (**HMAC-SHA256 under a key derived from `SESSION_TOKEN_SECRET` — never the raw number**, because this table takes a row for every number that ASKS for a code including numbers with no account, which raw would be the largest non-user PII pile on the platform), nullable `ip_hash`, `purpose`, `created_at`. ⚠️ **No FK to `users` by design** — counting requests for numbers with no account is the entire point. Rows purge after 48h; it is a counter, **not an audit log**. RLS on with zero policies = deny-all. |
...
| Password login option | ✅ | **LIVE AND PRODUCTION-PROVEN (2026-08-21), AND M10 IS NOW COMPLETE (2026-08-28).** Phone + password is the PRIMARY login; OTP is the fallback. Proven on the real LAN production build: password login reached the dashboard with real data loading (landing alone was not accepted — a token that mints but fails the ladder looks identical for half a second). The **mandatory set-password gate** ([I27]) forces every password-less account to set one before reaching the app, re-checked on EVERY app entry. Every layer proven in its own chunk: argon2id hashing (2.2) · set-password (2.4) · credential verify (2.5a) · our own token issue+verify+ladder (2.5b) · per-account lockout (2.7) · server-side throttled OTP send (2.6c, with the D4 timing floor) · send-path latency (2.6d) · reset (2.8a+2.8b) · **anti-spraying (2.10)**. ✅ **PASSWORD SPRAYING IS HANDLED since 2.10 (2026-08-27), production-proven** — `checkLoginSprayThrottle` blocks an address that has failed against **10 DISTINCT ACCOUNTS in 15 minutes**. ⚠️ **It is NOT per-IP rate limiting**, which [I23] refused because a naive attempt cap behind an office NAT locks out real users; it counts distinct FAILED accounts — a shape only a sprayer produces — and a **successful login clears that account's rows for that address**, so an office never trips it ([I35]). It **fails OPEN and loudly** ([I36]), because fail-closed would take the platform's whole login down on a database blip. The block returns a **generic 401, not a 429**, so a sprayer is never told they were detected. Proven live: a sprayed address blocked while a real user on another address logged in normally. ⚠️ **STILL OPEN — `/api/dev-auth/lookup` IS UNAUTHENTICATED** and returns `select("*")` on `users` for any phone. [I10] made a password-hash leak impossible by construction (separate table); it did NOT fix the route. ⚠️ **NEW-USER SIGNUP IS COVERED STRUCTURALLY BY THE [I27] GATE BUT HAS NEVER BEEN RUN END-TO-END ON A GENUINELY NEW ACCOUNT.** It is an inference from the gate's design, not an observation. Verify against a real new signup before recording signup-with-password as done. *(History, kept because it teaches: this row once read "password SPRAYING is unhandled … and 2.6a merged without the decision that was supposed to gate it." That was true from 2.6a until 2.10. The register carried the debt for six chunks before it was paid.)* |
| Password RESET (forgot password) | ✅ | **LIVE AND PRODUCTION-PROVEN 2026-08-27 (chunk 2.8b).** `/login` → Forgot password? → `/reset-password` → real SMS → code + new password → dashboard, run end to end on a real number against the LAN production build. `token_epoch` **0 → 1** (the eviction), no phantom account, no stray credential. **2.8 WAS SPLIT INTO 2.8a (the seam) + 2.8b (route + UI) — AND BOTH ARE DONE.** 2.8a: `resetPasswordByOtp` (`authProvider.server.ts`) + `resetUserCredential` (`db.ts`), landed 2026-08-21 (`267271c`, 40/40). 2.8b added the HTTP surface and the screen. ⚠️ **THE SUBMIT ENDPOINT IS THROTTLED ([I33])** — it is unauthenticated and gated only by a 6-digit code, and NOTHING counted verify attempts before 2.8b. Sends and guesses are counted separately by `purpose`, so a failed guess cannot burn the victim's send budget and lock them out of their own recovery. ⚠️ **AND `verifyOtp` NO LONGER RUNS ON THE SHARED ADMIN CLIENT ([I34])** — it saves a session on whatever client it touches, and supabase-js prefers a session token over the client's own key, so on `supabaseAdmin` it would have downgraded EVERY `db.ts` call in the process from service role to that one user. Confirmed held on its first real production execution. **What it does:** verifies the OTP **SERVER-SIDE** (OTP login verifies in the browser, and a reset has no session to check — "the browser says the code passed" would let anyone POST that claim for any number), resolves, validates policy, then writes the hash + **bumps `token_epoch`** + **clears the lockout** in ONE statement. ⚠️ **The epoch bump is the point, not bookkeeping** — without it a reset leaves every existing session alive and the owner recovers nothing. ⚠️ **IT EVICTS *OUR* TOKENS ONLY: a stolen SUPABASE session SURVIVES a reset**, and every OTP login still mints one — **never describe reset as "ends all your sessions."** ⚠️ **A reset response is held to the D4 timing floor of 5000 ms** (measured, not guessed — 2.6d re-measured the ceiling to 3621 ms and lowered the floor from 6000). A ~5 s reset is slow; going below it needs the single-query throttle rewrite, not more concurrency. ⚠️ **No email reset links, ever** ([I10]; no `reset_token` column) — we do not verify emails. ⚠️ **The account's security floor is the PHONE NUMBER** — a SIM swap takes the account regardless of password strength. Already true because OTP login exists; the password did not raise it. *(History, kept because it teaches: for several hours on 2026-08-21 every status doc said reset was unstarted, because `267271c` touched ZERO markdown and the 2.8a/2.8b split existed only inside a commit message. A session trusting the docs would have rebuilt a proven, security-critical function — a Prime Directive #1 violation caused by the docs. **git is ground truth for what EXISTS; these files are ground truth for what it MEANS.**)* |
...
- 🛑 **THREE GATES BEFORE MERGING TO `main`** (merging auto-deploys to Vercel):
  1. **Chunk 1.10's owed production session** — closes 1.8's first execution and 1.9's miss-then-fallback branch. Never run.
  2. **The repo-wide eslint decision** — **29 errors + 11 warnings** across 28 files, 27 of them `react-hooks/set-state-in-effect`, all lit up by an `eslint-plugin-react-hooks` v7.1.1 bump rather than by any code change. `CLAUDE.md` §3 requires a clean build and this is not one. Fix them, pin the plugin, or record the ruleset as accepted-and-deferred — but **decide**; "it was green before" is not a decision, it was green under a different plugin version.
  3. **A deliberate merge**, taken knowing it deploys.
- ⚠️ **A LOGIN OCCASIONALLY TAKES >15 SECONDS. CAUSE UNKNOWN — NOT REPRODUCED, NOT EXPLAINED.** (observed 2026-08-27) One of twelve sequential `POST /api/auth/password-login` requests exceeded a 15 s client timeout; the server completed it correctly, so nothing was lost. **Not an anti-spray bug** — the blocked path is the FAST path at 1.0 s, and six clean failed logins immediately afterwards ran 1.0–1.9 s. ⚠️ **"Provider latency spike" is the leading hypothesis and NOT a finding**; 15 s is far outside the 1.5–3.5 s spikes measured during 2.6d, and `next start` logs no per-request timings so there is no direct measurement. On Vercel this would meet a function timeout rather than merely feel slow. **The cheapest next step is per-request timing on the auth routes, so the next occurrence is measured instead of inferred.**
- ⚠️ **`/api/dev-auth/lookup` IS STILL UNAUTHENTICATED** and returns `select("*")` on `users` for any phone — name, email, city, verification status, to an anonymous caller. [I10] made a password-hash leak impossible by construction (separate table); **it did not fix the route.** Real-world risk is targeted phishing and competitor scraping of the supplier base, not account takeover. ⚠️ Fix with `getVerifiedCallerPhone`, **never** `getVerifiedUser` — the route must still answer for a first-time signup that has no `users` row yet.
- ⚠️ **A DATABASE OUTAGE ROUTES AN EXISTING USER INTO ONBOARDING.** `login/page.tsx` and `signup/page.tsx` wrap the lookup in `try/catch` and never check `res.ok`, so a failed lookup treats a returning user as brand new. Same phantom-account class as Issue A, still live in the login path. **Must be fixed before or with any `dev-auth/lookup` lockdown** — adding 401/503 responses there makes this failure mode more likely, not less.
- ⚠️ **The `SPRAY PATTERN` log line fires on EVERY blocked attempt**, not once per spray, so a sprayer hammering an already-blocked address produces one `console.error` per request. Harmless at current volume and useful as forensic evidence, but it is attacker-driven log volume.
- ⚠️ **NEW-USER SIGNUP WITH THE [I27] GATE HAS NEVER BEEN RUN END-TO-END** on a genuinely new account. Structurally covered, never observed.
- ⚠️ **A RESET DOES NOT EVICT A STOLEN SUPABASE SESSION** — the epoch bump evicts our tokens only, and every OTP login still mints a Supabase one. **Never write "reset ends all your sessions."**
- ⚠️ **The security floor of every account is the PHONE NUMBER.** A SIM swap takes the account regardless of password strength. True before M10; the password did not raise it.
