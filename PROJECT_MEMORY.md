...
| Password login option | 🟡 | decided (M10). **2.0+2.1 done 2026-08-06; 2.2+2.3+2.4+2.5a done 2026-08-08; 2.7 (lockout) done 2026-08-20.** ⚠️ **A user can SET a password, the server can CHECK one, and a brute-forcer gets LOCKED OUT after 10 tries — but nothing can LOG IN with one, and no screen reaches any of it.** The verifier `verifyPasswordCredential` has **zero route importers on purpose** ([I18]): an endpoint answering "are these credentials valid?" without issuing a session is a credential oracle, and its absence is what makes deferring lockout safe. Remaining: **2.5b** (issue/verify our own token), 2.6 (login UI), 2.8 (reset). ✅ **2.7 (lockout) landed 2026-08-20, ahead of 2.6 — that gate is satisfied:** 10 consecutive failures → a fixed 15-minute auto-expiring cooldown, per-account, zero DDL, 51/51 ([I23]–[I26]). ⚠️ **The locked state is disclosed ONLY to a caller who supplied the CORRECT password** ([I24], amending [I17]); a prober sees the byte-identical generic failure, proven by value, by fuzzing and by a 3-round-trip / ~46 ms cost floor on all five paths. ⚠️ **Password SPRAYING is still unhandled** — per-IP is deliberately unbuilt, and **2.6 must not merge without a decision there**. ⚠️ **2.5b is HALF-BUILT AND UNCOMMITTED:** the standalone token module is on disk untracked with passing suites (54/54, 72/72), but [I21]'s type widening and the resolution-ladder branch do not exist and no app file imports it. ⚠️ M10 requires us to issue our OWN session tokens (Supabase will not sign a JWT for a credential it does not hold), which pulls A12 Phase 2 dual-verify forward; roughly half of M10 is the token subsystem, not the credential. Highest-risk chunks: ✅ **2.2 (argon2id hashing — done, 42/42)** and 🔴 **2.5b (our own token verify — a bug is an auth bypass; the last one)**. |
