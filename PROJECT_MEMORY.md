...
| ⚠️ **Auth seam — WIRING IN PROGRESS (4 of 6 done)** | 🟡 | ✅ **`db.ts`** (1.5 — auth removed entirely) · ✅ **`auth.ts`** (1.5, production-proven) · ✅ **`login/page.tsx`** (1.6, production-proven, zero supabase refs) · ✅ **`signup/page.tsx`** (1.7, **proven by a real production signup on a fresh number**, zero supabase refs). 🔴 Remaining: **`UserContext`, `AuthGuard`, `apiClient`** (1.10). Supabase auth is now imported in **3** call sites, down from 6 — `apiClient.ts:75` was missed by the first audit and found in chunk 1.1. Until 1.10 this remains the largest migration risk we carry. ⚠️ `AuthGuard.tsx:143-144` splits `supabase.auth` / `.getSession()` across lines, so single-line greps miss it — don't use one to confirm 1.10 is done. ⚠️ `getSession()` was NOT needed by 1.6 or 1.7 (neither page calls it); its only two call sites are `AuthGuard.tsx:144` and `apiClient.ts:75`, both in 1.10. |
