...
| Password login option | 🔴 | decided (M10), **chunked into 2.0–2.9; 2.0 (decisions) + 2.1 (table) done 2026-08-06**. Nothing authenticates by password and nothing writes a credential yet — the first writer is chunk 2.4. ⚠️ M10 requires us to issue our OWN session tokens (Supabase will not sign a JWT for a credential it does not hold), which pulls A12 Phase 2 dual-verify forward; roughly half of M10 is the token subsystem, not the credential. Highest-risk chunks: **2.2 (argon2id hashing — fails silently and retroactively)** and **2.5 (our own token verify — a bug is an auth bypass)**. |
