...
function downloadPlaceholder(label: string, orderId: string) {
  const blob = new Blob(
    [
      `${label} — Order ${orderId}\n\nThis is a demo build. The buyer's uploaded file content is not stored in this environment.`,
    ],
    { type: "text/plain" }
  );
  ...
  link.href = url;
  link.download = `${label.replace(/\s+/g, "_")}.txt`;
  link.click();
...
function AcceptOrderModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[440px] rounded-xl border border-border-dark bg-card p-7"
        onClick={(event) => event.stopPropagation()}
      >
        <p className="text-lg font-bold text-white">
          Accept this order for ₹{order.totalValue.toLocaleString("en-IN")}?
        </p>
        <p className="mt-2 text-sm text-text-secondary">
          Production must begin immediately to meet the{" "}
        ...
        <div className="mt-6 flex flex-col gap-3">
          <button
            ...
            className="rounded-lg bg-green-500 py-3 text-sm font-bold text-navy"
          >
            Accept Order →
          </button>
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg border border-border-dark py-3 text-sm font-semibold text-text-secondary"
          >
            Request Changes
          </button>
          <button
  ...
  return (
    <div className="mb-4 rounded-[10px] border border-primary bg-card p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="rounded-[20px] border border-amber-500/60 bg-amber-500/15 px-3 py-1 text-xs font-semibold text-amber-400">
          Pending Acceptance
        </span>
      </div>

      <div className="mt-3">
        <p className="text-sm text-text-primary">From: {order.brandName}</p>
        <p className="mt-0.5 text-xs text-text-secondary">
          Value: ₹{order.totalValue.toLocaleString("en-IN")} • Delivery:{" "}
          {new Date(`${order.deliveryDate}T00:00:00`).toLocaleDateString("en-US", {
            month: "long",
            day: "numeric",
            year: "numeric",
      ...
      <div className="mt-4 flex flex-wrap gap-2 border-t border-border-dark pt-3">
        <button
          type="button"
        ...
        <button
          ...
          className="rounded-lg bg-green-500 px-3 py-1.5 text-xs font-bold text-navy"
        >
          Accept Order ✓
        </button>
      </div>
      ...
      {expanded && (
        <div className="mt-4 border-t border-border-dark pt-3">
          <p className="mb-2 text-xs font-bold uppercase tracking-wide text-text-secondary">
            Documents Uploaded by Buyer
          </p>
          <div className="flex flex-col gap-1.5">
            {order.documents.map((doc) => (
              <div key={doc.key} className="flex items-center justify-between text-sm">
                <span className={doc.uploaded ? "text-text-primary" : "text-text-secondary"}>
                  {doc.uploaded ? "✅" : "⚠"} {doc.label}
                </span>
                {doc.uploaded && (
                  <button
                    type="button"
                    onClick={() => downloadPlaceholder(doc.label, order.id)}
          ...
          <p className="mb-2 mt-4 text-xs font-bold uppercase tracking-wide text-text-secondary">
            Payment Milestones
          </p>
          <div className="flex flex-col gap-1.5">
            {order.paymentMilestones.map((m) => (
              <div key={m.name} className="flex justify-between text-sm">
                ...
                <span className="text-text-primary">
                  {m.percent}% — ₹{m.amount.toLocaleString("en-IN")}
                </span>
              </div>
            ))}
          </div>
...
export default function Orders() {
  ...
  const pendingBulkOrdersSection =
    user.userType === "manufacturer" && pendingBulkOrders.length > 0 ? (
      <div className="mb-6">
        {pendingBulkOrders.map((order) => (
          <PendingBulkOrderCard
            key={order.id}
            order={order}
            expanded={expandedBulkOrderId === order.id}
            onToggleExpand={() =>
              setExpandedBulkOrderId((current) => (current === order.id ? null : order.id))
            }
            onAcceptClick={() => setAcceptModalOrder(order)}
          />
        ))}
      </div>
    ) : null;
