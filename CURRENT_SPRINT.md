...
**➡️ WHAT IS LEFT, IN ORDER: 2.8b (reset route + UI + production test) → 2.9 (docs sweep).** Two chunks. ✅ **2.6c DONE and production-proven (2026-08-24); 2.6d DONE and production-measured (2026-08-26)** — registered reset 4722 → 2915 ms, floor 6000 → 5000, re-proven binding. ⚠️ **Still owed and easy to lose: the login-route anti-spraying chunk — its TASKS entry is STILL UNWRITTEN and it is a HARD MUST-DO BEFORE MERGE TO `main`.** **Nothing is deployed** — every M10 commit is local-only on `auth-hardening-batch`.
