...
**➡️ WHAT IS LEFT, IN ORDER: 2.6d (OTP send-path latency — HARD PREREQUISITE of 2.8b) → 2.8b (reset route + UI + production test) → 2.9 (docs sweep).** Three chunks. ✅ **2.6c is DONE and production-proven (2026-08-24).** **Nothing is deployed** — every M10 commit is local-only on `auth-hardening-batch`.
